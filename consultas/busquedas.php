...
if($tp == 'cte'){
    ...
    // $ficha = $_POST['ficha'];
    // $tblcomp="";
    // $rs=$query->Consultar("CT.CORTA","fficha FF INNER JOIN fcombinaciones CB ON CB.ID_FICHA=FF.ID_FICHA INNER JOIN fcat_combinacion CT ON CT.ID_CAT_COMBINACION=CB.ID_CAT_COMBINACION","FF.ID_FICHA=".$ficha."","");
    // if($rs->RecordCount()>0){
    //     $tblcomp.="<table id='tbl_lab1' class='table table-responsive table-hover'style='display: block; font-size: 15px'>";
    //     $tblcomp.="<thead><tr><th rowspan='2'>COMPOSICION(ES)</th><th colspan='5'>VARIANTES</th></tr><tr><th>1</th><th>2</th><th>3</th><th>4</th></tr>";
    //     while(!$rs->EOF){
    //         // $tblcomp.="<tr><tr id='row".$rs->fields['ID_COMBINACION']."'><th>".$rs->fields['CORTA']."</th><th class='cont_comp'><label><input class='i_responsive' type='number'>. % .</label><select class='s_responsive' name='0' id='0'>Composición</select><br></tr>";
    //         $tblcomp.="<th rowspan='5' >".$rs->fields['CORTA']."</th>";
    //         for($i=1;$i<=4;$i++){
                
    //             $tblcomp.= "<tr>";
    //             $tblcomp.= "<th colspan='0' class='cont_comp'><label><input class='i_responsive' type='number'>. % .</label>";
    //             $tblcomp.= "<select class='s_responsive' name='0' id='0'>Composición</select>";
    //             $tblcomp.= "</th>";
    //             $tblcomp.= "<th class='cont_comp'><label><input class='i_responsive' type='number'>. % .</label>";
    //             $tblcomp.= "<select class='s_responsive' name='0' id='0'>Composición</select>";
    //             $tblcomp.= "</th>";
    //             $tblcomp.= "<th class='cont_comp'><label><input class='i_responsive' type='number'>. % .</label>";
    //             $tblcomp.= "<select class='s_responsive' name='0' id='0'>Composición</select>";
    ...
    $ficha = $_POST['ficha'];
    $tbl = "";
    $rm = $query->Consultar("CB.ID_COMBINACION, CB.ID_CAT_COMBINACION, CT.CORTA,MAX(FV.VARIANTE) as NUM,FV.COLOR","FCOMBINACIONES CB RIGHT JOIN FCAT_COMBINACION CT ON CT.ID_CAT_COMBINACION = CB.ID_CAT_COMBINACION RIGHT JOIN fvariantes FV ON FV.ID_COMBINACION=CB.ID_COMBINACION","CB.ID_FICHA =".$ficha." AND CB.ID_CAT_COMBINACION NOT IN (7,8,11,12,13)","CB.ID_COMBINACION");
    if($rm->RecordCount()>0){
        $cont=$rm->fields['NUM'];
